# **Summary of things i learned using Oracle VirtualBox, free license Windows server 2025 and Active Directory in a home set up:**
- Learning basics about networks, and commonly used commands (ping, tracert, ipconfig etc.)
- Setting up VM`s with win11, windows server 2025 and ubuntu server
![VM](VMs.png)
- Setting up a server with a static ip configured manually
![Static ip](static ip config.png)
- Installing Active Directory with additional tools:
  1. Domain setup
  2. Managing user accounts and giving them various privilages
  3. Group policy managment
  4. Sharing folders
  5. Configuring a DHCP server and a custom ip range
  6. Configuring a DNS server
  7. Making a basic local domain website and making it accesible for users with IIS
